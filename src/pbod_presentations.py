 ...
 ("pbod_preferences", 0, mesh_load_window, [
    (ti_on_presentation_load,
      [
        ...
        (create_check_box_overlay, reg0, "mesh_checkbox_off", "mesh_checkbox_off"), #Lancer
        (position_set_y, pos1, ":inputs_y"),
        (overlay_set_position, reg0, pos1),
		(val_sub, ":inputs_y", ":cur_y_shift"),

        (create_check_box_overlay, reg0, "mesh_checkbox_off", "mesh_checkbox_off"), #Horse Archer
        (position_set_y, pos1, ":inputs_y"),
        (overlay_set_position, reg0, pos1),  
        (val_sub, ":inputs_y", ":cur_y_shift"),		

        (create_check_box_overlay, reg0, "mesh_checkbox_off", "mesh_checkbox_off"), #Spear/Polearm
        (position_set_y, pos1, ":inputs_y"),
        (overlay_set_position, reg0, pos1),  
		(val_sub, ":inputs_y", ":cur_y_shift"),
      ...
      [
		...
		(else_try),
		  ...
		  (party_set_slot, "p_main_party", slot_party_pref_wu_spear, 0),	
		(else_try),
		  (val_add, ":overlay", 1),
 ...
 ("caba_order_display", prsntf_read_only,0,[
      (ti_on_presentation_load,
       [(set_fixed_point_multiplier, 1000),
		...
		(try_end),
		(neg|party_slot_eq, "p_main_party", slot_party_gk_order, 99),
		...
		(call_script, "script_str_store_key_name", s16, "$key_order_10"), #F10
		...
		(try_begin), #Figure out which orders to display, set strings
			...
			(try_begin),
				...
				(str_store_string, s1, "@{s11} - Formation type orders"),
				(str_store_string, s2, "@{s12} - Weapon orders"),
				(str_store_string, s3, "@{s13} - Shield orders"),
				(str_store_string, s4, "@{s14} - Attack orders"),
				(str_store_string, s5, "@{s16} - Escape Menu"),
			...
			(else_try),
				...
				(str_store_string, s4, "@{s16} - Escape Menu"),
				(assign, ":num_orders", 4),
			(try_end),
			(assign, ":y_position", 443), #470
		(else_try),
		    (party_slot_eq, "p_main_party", slot_party_gk_order, gk_order_5),
			(str_store_string, s1, "@{s11} - Ranks"),
			(str_store_string, s2, "@{s12} - Shieldwall"),
            (str_store_string, s3, "@{s13} - Wedge"),
			(str_store_string, s4, "@{s14} - Square"),
			(str_store_string, s5, "@{s15} - No Formation"),
			(str_store_string, s6, "@{s16} - Escape Menu"),
		...
		(else_try),
            ...
            (assign, ":num_orders", 4),
		(else_try),
			(party_slot_eq, "p_main_party", slot_party_gk_order, "$key_order_8"),
			(str_store_string, s1, "@{s11} - End Order"),
			(str_store_string, s2, "@{s12} - Skirmish"),
			(str_store_string, s3, "@{s13} - Volley Fire"),
			(str_store_string, s4, "@{s14} - Brace Polearms"),
			(str_store_string, s5, "@{s16} - Escape Menu"),
	...
	(ti_on_presentation_run,
       [(store_trigger_param_1, ":cur_time"),
        ...
        (try_begin),
		  (this_or_next|key_clicked, "$key_order_10"), #current exit key
        ...
        (else_try),
			...
			(else_try),
				...
				(else_try),
					...
					(assign, ":max_key", 7),
				(else_try),
				    (party_slot_eq, "p_main_party_backup", slot_party_gk_order, "$key_order_8"),
					(assign, ":min_key", 5),
					(assign, ":max_key", 8),
				(try_end),
